...
def handle_message(event):
    ...
    if "material" not in state:
        ...
        if msg in mat_opts:
            state["material"] = msg
            img_url = MATERIAL_IMAGES.get(msg)
            next_msg = quick("請選擇坪數範圍：", PING_OPTIONS)
            if img_url:
                line_bot_api.reply_message(event.reply_token, [
                ...
                line_bot_api.reply_message(event.reply_token, next_msg)
            return
        else:
            text, show_menu = smart_reply(msg, state, "材質", mat_opts)
            line_bot_api.reply_message(event.reply_token,
                quick(text, mat_opts) if show_menu else TextSendMessage(text=text))
            return
